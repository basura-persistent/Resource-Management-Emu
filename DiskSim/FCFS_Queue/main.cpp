...
FCFSQueue *createFCFSQueueFromInputFile( int argc, char *argv[] ) {//this will return a queue nodes
    ...
    auto *queue = new FCFSQueue();//calling pointer object to FCFS queue so we can run all functions from FCFSQueue class
    ...
    while(numRequestsInQueue_input > 0){
        request_input = queue->getRequest();
        std::cout << "Removed the following request from the FCFSQueue via getRequest.\n";
        request_input->print();
        if(request_input->time() < previousTimeStamp_input)//since it is a first come first serve
            std::cout << "Error: The FCFSQueue just returned a request whose timestamp is out of order.\n";
        previousTimeStamp_input = request_input->time();
        delete request_input; // We do not have a use for this request any longer.
        numRequestsInQueue_input--;
    }
    cout<<numRequestsGenerated_input<<endl;
    return queue;
}
...
FCFSQueue *createFCFSQueueFromRandomValues(int numRequestsToGenerate, int numTestsToRun) {
    ...
    while ( numTestsRun < numTestsToRun ) {
        ...
        if (numRequestsGenerated < numRequestsToGenerate && actionRange >= ADD_A_REQUEST && actionRange < GET_A_REQUEST) { //creating our randomly generated set of requests
            ...
            if (queue->empty())
                ...
                std::cout << "Removed the following request from the FCFSQueue via getRequest.\n";
                request->print();
                if(request->time() < previousTimeStamp)//since it is a first come first serve
                    std::cout << "Error: The FCFSQueue just returned a request whose timestamp is out of order.\n";
    ...
    while(! queue->empty() ) {//once we we have run reached number of tests to run
...
int main(int argc, char *argv[]) {
    ...
    if( randomTest )
        queue = createFCFSQueueFromRandomValues(10, 10);//numtests to generate// num tests to run
    else
