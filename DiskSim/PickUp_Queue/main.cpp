...
PICKUPQueue *createPICKUPQueueFromInputFile( int argc, char *argv[] ) {//this will return a queue nodes
    ...
    auto *queue = new PICKUPQueue();//calling pointer object to FCFS queue so we can run all functions from FCFSQueue class
...
int main(int argc, char *argv[]) {
    ...
    if(randomTest){
        ...
        queue = createPICKUPQueueFromInputFile(argc, argv);
    }
    queue->print();
    delete queue;
